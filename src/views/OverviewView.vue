...
const kpiData = {
  todos: {
    vendas: 'R$ 84.320',
    ticket: 'R$ 1.248',
    svLabel: 'Este mês • todos os canais',
    stLabel: 'Por pedido • todos os canais',
  },
  b2b: {
    vendas: 'R$ 61.900',
    ticket: 'R$ 3.860',
    svLabel: 'Este mês • B2B Industrial',
    stLabel: 'Por pedido • B2B',
  },
  varejo: {
    vendas: 'R$ 22.420',
    ticket: 'R$   189',
    svLabel: 'Este mês • Varejo',
    stLabel: 'Por pedido • Varejo',
  },
}
...
const chartOptions = ref({
  ...
  scales: {
    x: {
      ticks: { color: '#415a77', font: { size: 16 } },
      grid: { color: '#001d3d' },
    },
    y: {
      min: 0,
...
const allOrders = ref([
  ...
  {
    id: '#4822',
    cli: 'Supermercado Rex',
    tipo: 'b2b',
    itens: 'Baguete Rústica 800un',
    val: 'R$ 1.440',
    urg: 'media',
    status: 'Aguardando',
    statusColor: '#778da9',
  ...
  {
    id: '#4823',
    cli: 'Padaria Doce Mel',
    tipo: 'varejo',
    itens: 'Croissant 40un',
    val: 'R$   180',
    urg: 'baixa',
    status: 'Entregue',
    statusColor: '#2dce89',
  ...
  {
    id: '#4824',
    cli: 'GPA Distribuidora',
    tipo: 'b2b',
    itens: 'Misto 12.000un',
    val: 'R$ 8.400',
    urg: 'alta',
    status: 'Produção',
    statusColor: '#c3a343',
...
const filteredOrders = computed(() => {
  const filter = filtroOrdem.value
  if (filter === 'urgente') return allOrders.value.filter((o) => o.urg === 'alta')
  if (filter === 'b2b') return allOrders.value.filter((o) => o.tipo === 'b2b')
  if (filter === 'varejo') return allOrders.value.filter((o) => o.tipo === 'varejo')
  if (filter === 'pendente') return allOrders.value.filter((o) => o.status !== 'Entregue')
    ...
    <main class="db-main">
        ...
        <div class="topbar-right">
          <div class="canal-tabs">
            <button
              ...
              @click="canalAtivo = c"
            >
              {{ c === 'todos' ? 'Todos' : c === 'b2b' ? 'B2B Industrial' : 'Varejo' }}
            </button>
          </div>
      ...
      <div class="kpi-grid">
        ...
        <div class="kpi kpi-yellow">
          <div class="kpi-icon kpi-icon--yellow">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M12 2c-5.5 0-9 4-9 9s3.5 9 9 9 9-4 9-9-3.5-9-9-9z"/>
              <path d="M12 6v6l4 2"/>
            </svg>
          </div>
      ...
      <div class="mid-row">
        <div class="card-box">
          ...
          <div class="prod-grid">
            <div v-for="produto in produtosVisiveis" :key="produto.id" class="prod-card">
        ...
        <div class="card-box">
          ...
          <div class="chart-legend">
            <span class="leg-item">
              <span class="leg-dot" style="background:#c3a343"></span>Utilizada
            </span>
            <span class="leg-item">
              <span class="leg-dot" style="background:#003566"></span>Disponível
            </span>
          </div>
          ...
          <table class="db-table">
            ...
            <tbody>
              <tr v-for="o in filteredOrders" :key="o.id">
                ...
                <td>
                  <span class="urg" :class="`urg--${o.urg}`">
                    {{ urgLabel[o.urg] }}
                  </span>
                </td>
                <td>
